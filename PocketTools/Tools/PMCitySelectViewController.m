...
@interface PMCitySelectViewController ()

@end

@implementation PMCitySelectViewController {
...
- (void)loadCity {
    [self displayHUD:@"加载中..."];
   [[ServiceRequest shared] loadPMCityListWithBlock:^(NSDictionary *result, NSError *error) {
       if (!error) {
...
- (void)selectCity:(NSInteger)index {
    if ([self.delegate respondsToSelector:@selector(selectCityName:)]) {
        NSString *name = cityArray[index][@"name"];
