...
- (void)openPasscodeWithMode:(int)mode viewController:(UIViewController *)viewController {
    ...
    if (_mode == 0) {
        [_passcodeViewController setInstructions:NSLocalizedString(@"输入新密码", nil)];
    } else if (_mode == 1) {
        [_passcodeViewController setInstructions:NSLocalizedString(@"输入密码解锁", nil)];
    }
}
...
- (void)enteredCode:(NSString *)code {
    if (_mode == 0) {
        if (_count == 0) {
            _prevCode = code;
            [_passcodeViewController setInstructions:NSLocalizedString(@"请再次输入密码", nil)];
            ...
            if ([code isEqualToString:_prevCode]) {
                ...
                [_passcodeViewController setInstructions:NSLocalizedString(@"输入新密码", nil)];
                [_passcodeViewController setErrorMessage:NSLocalizedString(@"密码不对请再次输入", nil)];
                [_passcodeViewController reset];
                _count = 0;
                return;
            }
